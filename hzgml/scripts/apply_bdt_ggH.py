...
def getArgs():
    ...
    parser.add_argument('-c', '--config', action='store', nargs=2, default=['data/training_config_BDT_ggH.json', 'data/apply_config_BDT_ggH.json'], help='Region to process')
    parser.add_argument('-i', '--inputFolder', action='store', default='/eos/user/q/qguo/vbfhmm/ml/RunIII/skimmed_ntuples_ggH_v1/', help='directory of training inputs')
    ...
    parser.add_argument('-J', '--outJson', action='store', default='data/inputs_config_2223.json', help='Region to process')
...
class ApplyXGBHandler(object):
    ...
    def __init__(self, configPath, region=''):
        ...
        #self._inputTree = self.inputTree
        self._inputTree = ''
    ...
    def applyBDT(self, category, scale=1):
        ...
        branches += ["eventWeight", "trg_single_mu24", "nmuons", "diMufsr_rc_phi"]
        outputbraches += ["eventWeight", "trg_single_mu24", "nmuons", "diMufsr_rc_phi"]
