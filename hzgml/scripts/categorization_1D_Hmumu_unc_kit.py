...
def gettingsig(input_path, region, variable, boundaries, transform, arctanh_, estimate):
    ...
    for category in ['sig', 'vbf', "bkgmc_sid", "bkgmc_cen", "sig_tot"]:#['sig', 'vbf', 'ggH', "data_sid", "bkgmc_sid", "bkgmc_cen", "sig_tot"]
        # for data in tqdm(read_root(f'{input_path}/{region}/{"bkgmc" if "bkgmc" in category else "data" if "data" in category else "sig" if "sig" in category else category}.root', key='data_two_jet_m110To150', columns=[f"{variable}_score{'_t' if transform else ''}", 'diMufsr_kit_BSC_mass', 'eventWeight', 'event'], chunksize=500000), desc=f'Loading {category}', bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
        ...
        columns = [f"{variable}_score{'_t' if transform else ''}{'_arctanh' if arctanh_ else ''}", 'diMufsr_kit_BSC_mass', 'eventWeight', 'event']

        # Iterate over the data in chunks
        for data in tqdm(file['data_two_jet_m110To150'].iterate(columns, library='pd', step_size=500000), desc=f'Loading {category}', bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
    
            if 'sid' in category:
                data = data[(data.diMufsr_kit_BSC_mass >= 110) & (data.diMufsr_kit_BSC_mass <= 150) & ((data.diMufsr_kit_BSC_mass < 115) | (data.diMufsr_kit_BSC_mass > 135))]
                data['w'] = data.eventWeight

            elif 'tot' in category:
                data = data[(data.diMufsr_kit_BSC_mass >= 110) & (data.diMufsr_kit_BSC_mass <= 150)]
                data['w'] = data.eventWeight

            else:
                data = data[(data.diMufsr_kit_BSC_mass >= 115) & (data.diMufsr_kit_BSC_mass <= 135)]
                data['w'] = data.eventWeight
...
def categorizing(input_path, region, variable, sigs, bkgs, nscan, minN, transform, arctanh_, nbin, floatB, n_fold, fold, earlystop, estimate):
    ...
    t_sig = f_sig.Get('data_two_jet_m110To150')
 
    if estimate in ["fullSim", "fullSimrw"]:
        f_bkgmc = TFile('%s/%s/bkgmc.root' % (input_path, region))
        t_bkgmc = f_bkgmc.Get('data_two_jet_m110To150')
    if estimate in ["fullSimrw", "data_sid"]:
        f_data_sid = TFile('%s/%s/data.root' % (input_path, region))
        t_data_sid = f_data_sid.Get('data_two_jet_m110To150')

    h_sig = TH1F('h_sig','h_sig',nscan,0,1)
    h_bkg = TH1F('h_bkg','h_bkg',nscan,0,1)

    t_sig.Draw(f"{variable}_score{'_t' if transform else ''}{'_arctanh' if arctanh_ else ''}>>h_sig", "eventWeight*%f*((diMufsr_kit_BSC_mass>=115&&diMufsr_kit_BSC_mass<=135)&&(event%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))

    # filling bkg histograms
    if estimate in ["fullSim", "fullSimrw"]:
        h_bkgmc_cen = TH1F('h_bkgmc_cen', 'h_bkgmc_cen', nscan, 0., 1.)
        t_bkgmc.Draw(f"{variable}_score{'_t' if transform else ''}{'_arctanh' if arctanh_ else ''}>>h_bkgmc_cen", "eventWeight*%f*((diMufsr_kit_BSC_mass>=115&&diMufsr_kit_BSC_mass<=135)&&(event%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
    if estimate in ["fullSimrw"]:
        h_bkgmc_sid = TH1F('h_bkgmc_sid', 'h_bkgmc_sid', nscan, 0., 1.)
        t_bkgmc.Draw(f"{variable}_score{'_t' if transform else ''}{'_arctanh' if arctanh_ else ''}>>h_bkgmc_sid", "eventWeight*%f*((diMufsr_kit_BSC_mass>=100&&diMufsr_kit_BSC_mass<=180)&&!(diMufsr_kit_BSC_mass>=115&&diMufsr_kit_BSC_mass<=135)&&(event%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
    if estimate in ["fullSimrw", "data_sid"]:
        h_data_sid = TH1F('h_data_sid', 'h_data_sid', nscan, 0., 1.)
        t_data_sid.Draw(f"{variable}_score{'_t' if transform else ''}{'_arctanh' if arctanh_ else ''}>>h_data_sid", "eventWeight*%f*((diMufsr_kit_BSC_mass>=100&&diMufsr_kit_BSC_mass<=180)&&!(diMufsr_kit_BSC_mass>=115&&diMufsr_kit_BSC_mass<=135)&&(event%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
...
def main():
    ...
    #sigs = ["VBFHToMuMu_M125_ggHUnc", "GluGluHToMuMu_M125_ggHUnc"]
    ...
    #2022EE
    bkgs = ["DY_105To160_reweighted","EWK_LLJJ_M105To160","ST_tW_top","ST_tW_antitop","TTTo2L2Nu"]
    bkgs = ["DY_105To160_ZpT-reweighted","EWK_LLJJ_M50","ST_tW_top","ST_tW_antitop","TTTo2L2Nu"]
    # 2024
    bkgs = ["DY_105To160_ZpT-reweighted","EWK_LLJJ_M105To160","ST_tW_top","ST_tW_antitop","TTTo2L2Nu"]
