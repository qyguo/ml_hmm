...
#    for data in tqdm(read_root(args.input, key='DiMuonNtuple', columns=variables, chunksize=args.chunksize), desc='Processing %s' % args.input, bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
    ...
    data_two_jet_m110To150 = df[(df['njets'] >= 2) & (df['nleptons'] < 3) &(df['diMufsr_rc_mass'] > 110) &(df['diMufsr_rc_mass'] < 150) &(df['trg_single_mu24']) &(df['nbjets_loose'] <= 1) &(df['nbjets_medium'] <= 0) &(df['nmuons'] == 2) &(df['nelectrons'] == 0) &(df['jet1_eta'] <= 2.4) &(df['jet2_eta'] <= 2.4) &(df['jet1_pt'] >= 35) &(df['jet2_pt'] >= 25) &(df['dijet_mass'] >= 400) &(df['dijet_eta'] >= 2.5)]
